...
def bench_attention(config, output_path="results/attention.xlsx"):
    """Benchmark attention sub-ops for each [b, s, h, nh] combination."""
    ...
    for b, s, h, nh in tqdm(combos, desc="Attention"):
        if h % nh != 0:
            continue
        head_dim = h // nh
        ...
        score_gb = estimate_memory_gb(b * nh, s, s)
        ...
        if oom:
            ...
            for op in ("qk_matmul", "softmax", "score_v_matmul"):
                results.append({
                    "op_name": op,
                    "b": b, "s": s, "h": h,
                    "num_heads": nh, "head_dim": head_dim,
                    "time_ms": "OOM",
                })
            continue

        # Build tensors in attention layout: [b, nh, s, head_dim]
        Q = torch.randn(b, nh, s, head_dim, dtype=dtype, device=device)
        K = torch.randn(b, nh, s, head_dim, dtype=dtype, device=device)
        V = torch.randn(b, nh, s, head_dim, dtype=dtype, device=device)

        # --- Q @ K^T ---
        def qk_matmul(Q=Q, K=K):
            torch.bmm(
                Q.view(b * nh, s, head_dim),
                K.view(b * nh, s, head_dim).transpose(1, 2),
            )
        ...
        # --- Softmax ---
        scores = torch.randn(
            b, nh, s, s, dtype=dtype, device=device
        )
        ...
        def score_v_matmul(scores=scores, V=V):
            torch.bmm(
                scores.view(b * nh, s, s),
                V.view(b * nh, s, head_dim),
            )
